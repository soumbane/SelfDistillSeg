...
def loadBoun(data_dir: str, img_size: Tuple[int, ...]=(96,96,96), scale_intensity_ranged: bool = True, train_split: int = 4, show_verbose: bool = False) -> Tuple[CacheDataset, CacheDataset, int]:
    ...
    '''loader = LoadImaged(keys=("image","label","dist_map"))
    adc = AddChanneld(keys=["image", "label"])
    acf = AsChannelFirstd(keys=("dist_map")) # same as data_dict['dist_map'].permute(3, 0, 1, 2)
    cv = ConvertLabel(keys='label')
    spc = Spacingd(
            keys=["image", "label"],
            pixdim=(1.5, 1.5, 2.0),
            mode=("bilinear", "nearest"),
        )
    val_tr = Compose([loader,adc,acf,cv,spc])
    data_dict = val_tr(val_data_dicts[1])
    ...
    data_dict = train_transforms(train_data_dicts[0])
